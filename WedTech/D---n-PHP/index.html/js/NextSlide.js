...
var width_topSale__slider =
  document.querySelector(".topSale__slider").clientWidth;
// tong so anh la 8 = 2000px ; tức là lấy width tất cả ảnh
var width_all_img =
  topSale__slides.getElementsByTagName("img").length * kichthuocanh;
...
manualBtn.forEach(function (item, index) {
  item.onclick = function () {
    document
      .querySelector(".manual-checked__btn.set_background--color")
      .classList.remove("set_background--color");
    this.checked = true;
...
setInterval(function () {
  ...
  if (count === 1) {
    document
      .querySelector(".btn" + count)
      .classList.add("set_background--color");
    if (
      document
        .querySelector(".btn" + 4)
        .classList.contains("set_background--color") === true
    )
      document
        .querySelector(".btn" + 4)
        .classList.remove("set_background--color");
...
setInterval(function () {
  ...
  if (manual_checked) manual_checked.style.width = headerSliderWidth + "px";
